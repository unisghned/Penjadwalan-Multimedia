...
// --- 1. GOOGLE AUTH SYSTEM ---
function loginGoogle() {
    auth.signInWithPopup(provider).catch(error => {
        securityAlert("Gagal login: " + error.message);
    });
}
...
auth.onAuthStateChanged(user => {
    ...
    if (user) {
        ...
        renderSchedules(); // Pastikan jadwal di-load setelah login
    } else {
...
db.ref('members').on('value', snap => {
    ...
    snap.forEach(child => {
        ...
        grid.innerHTML += `<div class="bg-white dark:bg-[#162238] p-5 rounded-3xl border border-slate-200 dark:border-slate-700 text-center relative group animate__animated animate__fadeIn shadow-sm">
            <p class="font-bold text-slate-800 dark:text-white">${name}</p>
            <button onclick="hapusAnggota('${id}','${name}')" class="absolute top-2 right-2 text-red-500 opacity-0 group-hover:opacity-100 transition-all">🗑️</button>
        </div>`;
...
function setFilter(type) {
    ...
    if(type === 'Besar') {
        ...
        container.innerHTML = `<select id="input-jam" class="w-full bg-slate-50 dark:bg-[#0b1426] border border-slate-200 dark:border-slate-600 p-3 rounded-2xl text-sm outline-none focus:ring-2 ring-cyan-500 text-slate-900 dark:text-white transition-all">
        ...
        snap.forEach(child => {
            ...
            if(data.petugas) {
                Object.keys(data.petugas).forEach(k => {
                    petugasHtml += `<span class="bg-slate-50 dark:bg-[#0b1426] border border-slate-200 dark:border-slate-700 px-3 py-1.5 rounded-full text-[10px] text-cyan-700 dark:text-cyan-400 font-black animate__animated animate__fadeIn">
            ...
            list.innerHTML += `
                <div class="bg-white dark:bg-[#162238] p-6 rounded-[2rem] flex flex-col md:flex-row md:items-center justify-between gap-6 shadow-xl border border-slate-200 dark:border-slate-700 animate__animated animate__fadeInUp transition-all">
                    <div class="flex-1">
                        <span class="text-[8px] font-black px-2 py-0.5 rounded bg-cyan-600 text-white uppercase tracking-tighter">${data.kategori}</span>
                        <h4 class="text-2xl font-black text-slate-800 dark:text-white mt-1 uppercase tracking-tighter">${data.jam}</h4>
                        <p class="text-[10px] text-slate-400 font-bold uppercase tracking-widest">${data.namaMisa} • ${data.tanggal}</p>
                    </div>
                    ...
                    <div class="flex items-center gap-4 justify-end min-w-[160px]">
                        ...
                        <button onclick="openDaftar('${child.key}','${data.jam}')" class="bg-cyan-600 text-white px-8 py-3 rounded-2xl text-xs font-black shadow-lg shadow-cyan-600/20 active:scale-95 transition-all ${count>=lim?'opacity-30':''}" ${count>=lim?'disabled':''}>
...
function openDaftar(id, jam) { 
    ...
    document.getElementById('modal-info').innerText = jam; 
...
function hapusSatuJadwal(id) { 
    Swal.fire({ 
        title: 'Hapus?', icon: 'warning', showCancelButton: true, confirmButtonColor: '#d33', 
        background: document.documentElement.classList.contains('dark') ? '#162238' : '#fff' 
    }).then(r => { if(r.isConfirmed) db.ref(`schedules/${id}`).remove(); }); 
}

function removePetugas(s, k) { 
    db.ref(`schedules/${s}/petugas/${k}`).remove(); 
}

function resetSemuaJadwal() { 
    if(prompt("Ketik 'HAPUS' untuk meriset:") === "HAPUS") db.ref('schedules').set(null); 
}
